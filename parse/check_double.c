...
/*   Updated: 2021/02/27 13:32:13 by lpascrea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_check_double(char *buf, t_parse *parse)
{
	...
	&& (parse->i != parse->north && parse->north > 0))
		return (0);
	if ((buf[parse->i] == 'S' && buf[parse->i + 1] == 'O')
	&& (parse->i != parse->south && parse->south > 0))
		return (0);
	if ((buf[parse->i] == 'W' && buf[parse->i + 1] == 'E')
	&& (parse->i != parse->west && parse->west > 0))
		return (0);
	if ((buf[parse->i] == 'E' && buf[parse->i + 1] == 'A')
	&& (parse->i != parse->east && parse->east > 0))
		return (0);
	if ((buf[parse->i] == 'S' && buf[parse->i + 1] == ' ')
	&& (parse->i != parse->sprite && parse->sprite > 0))
		return (0);
